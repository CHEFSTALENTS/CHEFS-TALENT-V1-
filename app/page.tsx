  ...
  return (
    <Layout>
      ...
      <section className="relative min-h-screen grid md:grid-cols-12 pt-24 md:pt-0 bg-paper">
            ...
            <Reveal>
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <Link href="/private-clients">
                  <Button variant="outline" size="lg" className="w-full sm:w-auto border-stone-300 text-stone-600 hover:text-stone-900">
        ...
        <div className="md:col-span-5 relative min-h-[50vh] md:min-h-screen order-1 md:order-2 overflow-hidden">
           <motion.div 
           ...
           >
             <img 
               src="https://images.unsplash.com/photo-1626202378955-46d5c654316d?q=80&w=1974&auto=format&fit=crop" 
      ...
      <Section className="bg-paper">
        <div className="grid md:grid-cols-12 gap-24 items-center">
          ...
          <div className="md:col-span-6 space-y-16">
            <Reveal>
              <Marker />
              <h2 className="text-4xl md:text-5xl font-serif mb-8 text-stone-900 font-normal leading-tight">Deux niveaux de sélection, selon vos exigences.</h2>
      ...
      <section className="relative py-40 md:py-64 bg-stone-900 text-stone-100 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-6">
           <Reveal>
             ...
             <Link href="/request">
               <Button className="bg-paper text-stone-900 border-none hover:bg-white px-12 h-16">Soumettre une demande</Button>
             </Link>
           </Reveal>
        </div>
      </section>
...
const InteractiveCard = ({ theme, icon: Icon, title, subtitle, description, link, cta }: any) => {
  ...
  return (
    <div className={`group relative flex flex-col justify-between p-12 md:p-16 transition-all duration-700 border border-transparent hover:border-stone-200 ${isDark ? 'bg-[#2E2B26] text-stone-100' : 'bg-paper text-stone-900'}`}>
      <div className="relative z-10 space-y-8">
        <Icon className={`h-6 w-6 ${isDark ? 'text-bronze' : 'text-stone-400'}`} strokeWidth={1} />
        
        <div className="space-y-2">
          <p className={`text-[10px] uppercase tracking-[0.25em] font-medium ${isDark ? 'text-stone-500' : 'text-stone-400'}`}>{subtitle}</p>
        ...
        <div className="max-w-md">
          ...
          <Link href={link}>
             <span className={`inline-flex items-center text-sm font-medium border-b pb-1 transition-all ${isDark ? 'border-stone-600 text-stone-400 hover:text-white hover:border-white' : 'border-stone-300 text-stone-500 hover:text-stone-900 hover:border-stone-900'}`}>
...
const ExpandableItem = ({ title, subtitle, content, isDefaultOpen = false }: { title: string, subtitle?: string, content: string, isDefaultOpen?: boolean }) => {
  ...
  return (
    <div className="border-b border-stone-200">
      <button 
        ...
        className="flex items-center justify-between w-full text-left group py-8"
      >
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
          <h3 className="text-3xl font-serif font-normal text-stone-400 group-hover:text-stone-900 transition-colors">{title}</h3>
          {subtitle && <span className="text-xs uppercase tracking-widest text-stone-500 bg-stone-100 px-2 py-1">{subtitle}</span>}
        </div>
        <span className={`text-xl font-light text-stone-300 transition-transform duration-500 ${isOpen ? 'rotate-45' : ''}`}>+</span>
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            <div className="pb-8 pl-4 border-l border-stone-200 ml-2">
              <p className="text-stone-600 text-lg font-light leading-relaxed max-w-2xl">
                {content}
              </p>
            </div>
          </motion.div>
