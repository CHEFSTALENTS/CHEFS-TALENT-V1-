...
export default function AdminDashboardPage() {
  ...
  // ✅ AJOUT: counts (sinon counts.xxx crash)
  const counts = useMemo(() => {
    ...
    return { b2bNew, b2cNew, inReview, chefsToValidate };
  }, [requests, chefs]);
  ...
  const quickLists = useMemo(() => {
    ...
    const b2bUrgent = byDateDesc(requests.filter(r => r.userType === 'b2b' && r.status === 'new'))
      .slice(0, 5)
      .map<QuickItem>(r => ({
        id: r.id,
        title: `B2B — ${r.location || 'Lieu non précisé'}`,
        subtitle: r.contact?.company ?? r.contact?.name ?? 'Concierge',
  ...
  // ✅ IMPORTANT: ton JSX doit être DANS un return
  return (
    <div className="p-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Cockpit Chef Talents — demandes, chefs, missions.</p>
        </div>
