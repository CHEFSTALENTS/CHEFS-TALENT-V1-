  ...
  const quickLists = useMemo(() => {
    ...
    const recentActivity: QuickItem[] = [
      ...byDateDesc(requests).slice(0, 4).map(r => ({
        ...
        subtitle: `${r.status} • ${r.mode || 'standard'}`,
      ...
      .sort((a, b) => new Date(b.meta || 0).getTime() - new Date(a.meta || 0).getTime())
      .slice(0, 8);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <Panel
                ...
                right={
                  <Link href="/admin/requests?status=new" className="text-sm text-stone-600 hover:text-stone-900">
                    Ouvrir l’inbox →
                  </Link>
                }
              >
              ...
              <Panel
                title="Modules"
                subtitle="Accès rapide aux pages clés"
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <BigLink href="/admin/requests?status=new" title="Demandes" desc="Inbox & détail demandes" icon={<IconInbox />} />
                  <BigLink href="/admin/chefs" title="Chefs" desc="Validation • score • profils" icon={<IconChefHat />} />
                  <BigLink href="/admin/proposals" title="Proposals / Missions" desc="Offers & suivi missions" icon={<IconBolt />} />
                </div>
              </Panel>
            </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {item.subtitle && <div className="text-xs text-stone-500 truncate mt-0.5">{item.subtitle}</div>}
                          </div>
                          <div className="text-xs text-stone-400 whitespace-nowrap">{item.meta || ''}</div>
                        </div>
                      </Link>
                    ))
                  )}
...
function StatCard({
  ...
  return (
    <Link
      href={href}
      className={`border rounded-xl p-4 hover:shadow-sm transition ${toneCls}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
...
function IconBuilding() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-stone-700">
      <path d="M4 20V6a2 2 0 0 1 2-2h6v16M14 8h4a2 2 0 0 1 2 2v10M8 8h2M8 12h2M8 16h2M16 12h2M16 16h2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
function IconUser() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-stone-700">
      <path d="M20 21a8 8 0 1 0-16 0M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
function IconSpark() {
...
function IconShield() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-stone-700">
      <path d="M12 2 20 6v7c0 5-3.5 9-8 9s-8-4-8-9V6l8-4Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  );
}
function IconInbox() {
