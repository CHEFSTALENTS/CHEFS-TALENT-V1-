...
export default function RequestsClient() {
  ...
  const filtered = useMemo(() => {
    return requests
      .filter(r =>
        type === 'b2b' ? r.userType === 'b2b'
        : type === 'b2c' ? r.userType !== 'b2b'
        : true
      )
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="p-3">
                    {r.contact?.name || '—'}
                    <div className="text-xs text-stone-400">{r.contact?.email || ''}</div>
                  </td>
                  <td className="p-3">{r.location || '—'}</td>
                  <td className="p-3">{r.dates?.start || '—'} → {r.dates?.end || '—'}</td>
              ...
              {filtered.length === 0 && (
                <tr><td className="p-3" colSpan={6}>Aucune demande.</td></tr>
              )}
            </tbody>
          </table>
        </div>
