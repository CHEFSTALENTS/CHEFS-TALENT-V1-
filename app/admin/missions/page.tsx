    ...
    <div className="space-y-4">
      ...
      <div className="md:hidden space-y-2">
        {loading ? (
          ...
          <div className="px-4 py-6 text-sm text-red-300 border border-red-500/30 rounded-2xl bg-red-500/5 text-center">{error}</div>
        ) : view.length === 0 ? (
          <div className="px-4 py-6 text-sm text-white/60 border border-white/10 rounded-2xl bg-white/5 text-center">Aucune mission dans cette catégorie.</div>
        ) : (
          view.map((m) => {
            const phase = computeMissionLifecyclePhase({
            ...
            return (
              <div
                key={m.id}
                className={`border rounded-2xl ${needsChefPay ? 'border-red-500/30 bg-red-500/[0.06]' : 'border-white/10 bg-white/5'}`}
              >
                <Link href={`/admin/missions/${encodeURIComponent(m.id)}`} className="block p-4 hover:bg-white/[0.05] transition rounded-2xl">
                  <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <PhaseBadge phase={phase} />
                      <PaymentBadge status={m.paymentStatus} />
                    </div>
                    <div className="text-xs text-white/55 whitespace-nowrap">{formatDate(m.startAt)}</div>
                  </div>
                  <div className="text-white font-medium leading-tight">{shortText(m.chefName, 40)}</div>
                  <div className="text-xs text-white/55 mt-0.5 truncate">{m.location} · {m.guestCount ?? '—'} pers</div>
                  <div className="text-xs text-white/85 mt-1 font-mono">
                    {m.chefAmount != null ? `${m.chefAmount.toLocaleString('fr-FR')} € chef` : '—'}
                    {needsChefPay && <span className="text-red-300 ml-2">⚠ non payé</span>}
                  </div>
                </Link>
                {/* CTA selon état */}
                {(m.status === 'confirmed' && m.paymentStatus === 'pending') || m.paymentStatus === 'paid' ? (
                  <div className="px-4 pb-3 pt-1 border-t border-white/10 flex flex-wrap gap-2">
                    {m.status === 'confirmed' && m.paymentStatus === 'pending' && (
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg border border-emerald-500/30 bg-emerald-500/15 text-sm text-emerald-100 hover:bg-emerald-500/25 transition"
                      >
                    ...
                    {m.paymentStatus === 'paid' && (
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg border border-white/10 bg-white/5 text-xs text-white/70 hover:bg-white/10 transition disabled:opacity-50"
                      >
                        {revertingId === m.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}
                        Annuler encaissée
                      </button>
                    )}
                  </div>
        ...
        <div className="text-xs text-white/45 px-2">{view.length} mission(s)</div>
      </div>
