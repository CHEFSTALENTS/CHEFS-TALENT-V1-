...
const Request = () => {
  ...
  useEffect(() => {
    ...
    if (modeParam === 'fast' || modeParam === 'concierge') {
      ...
      setMode(m);
      setFormData(prev => ({ ...prev, mode: m })); // Update Form Data Mode
  ...
  const selectMode = (selected: RequestMode) => {
    ...
    navigate(`?mode=${selected}`, { replace: true });
    setStep(1);
  };
  ...
  if (result) {
    const isFastMode = result.mode === 'instant_match'; // Mapped from backend response
        ...
        <div className="md:col-span-3">
          <div className="sticky top-32 space-y-8">
            ...
            <div className="space-y-2">
              <Link to="/request" onClick={() => setMode(null)} className="text-[10px] uppercase tracking-[0.2em] text-stone-400 hover:text-stone-900 transition-colors">
                ← Changer de mode
              </Link>
