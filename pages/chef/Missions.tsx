...
import { Briefcase, MapPin, Calendar, Users, Clock, Check, X, Loader2, MessageCircle } from 'lucide-react';
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="border-b border-stone-200 flex gap-8">
           <TabButton 
             ...
             count={missions.filter(m => m.status === 'offered').length}
           />
           <TabButton 
             label="En cours" 
             active={activeTab === 'active'} 
             onClick={() => setActiveTab('active')} 
             count={missions.filter(m => ['accepted', 'confirmed'].includes(m.status)).length}
           />
           <TabButton 
             label="Historique" 
...
const TabButton = ({ label, active, onClick, count }: { label: string, active: boolean, onClick: () => void, count?: number }) => (
  <button 
    onClick={onClick}
    className={`pb-4 text-sm font-medium tracking-wide uppercase transition-colors relative ${active ? 'text-stone-900' : 'text-stone-400 hover:text-stone-600'}`}
  >
    {label}
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-start gap-6">
         ...
         <div className="space-y-4 flex-grow">
            <div className="flex items-center gap-3">
               ...
               <span className="text-xs text-stone-400 uppercase tracking-widest">{new Date(mission.createdAt).toLocaleDateString()}</span>
            </div>
         ...
         {/* Actions or Amount */}
         <div className="md:text-right flex flex-col items-start md:items-end gap-4 min-w-[140px]">
            <div>
            ...
            {isOffer && (
              <div className="flex flex-col gap-2 w-full">
                 ...
                 >
                   {isActionLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <><Check className="w-3 h-3 mr-2" /> Accepter</>}
                 </Button>
                 <Button 
                   size="sm" 
                   variant="outline"
            ...
            {isConfirmed && mission.clientPhone && (
               <Button 
                  ...
                  onClick={() => {
                     ...
                     const message = `Bonjour, je suis votre chef via Chef Talents pour la mission du ${new Date(mission.startDate).toLocaleDateString()} à ${mission.location}. Je reste à votre disposition pour finaliser les détails.`;
